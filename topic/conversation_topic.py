...
while que != 'exit .':
    ...
    # predout_dbs, prob_dbs = dbs_decoder(Q[0:1], 10, 10, 0.3)
    # response_dbs = predout_dbs.strip().split('EOS')[0]
    # response_dbs = response_dbs.split('BOS')[1]
    # # print(response)
    # text = preprocess(response_dbs, name)
    # print ('computer (dbs): ' + text + '    (with probability of %f)'%prob_dbs)
    ...
    # text = preprocess(response_greedy, name)
    # print ('computer (greedy): ' + text + '    (with probability of %f)'%prob_greedy)
