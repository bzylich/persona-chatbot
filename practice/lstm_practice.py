...
def create_input_sequences(conv):
	...
	for i in range(len(conv)):
		...
		for j in range(max(0, i - max_len + 1), i + 1):
			new_seq.append(conv[j])
		new_conv.append(new_seq)
	return new_conv

conv1_padded = pad_sequences(create_input_sequences(conv1_tokenized), padding="pre", value=vocab['<unk>'])
...
model = Sequential()
model.add(Embedding(len(vocab), 256, input_length=max_len))
...
if len(sys.argv) < 2 or not os.path.exists(filepath):
	model.fit(X_train, y_train, epochs=400, batch_size=batch_size, verbose=1, callbacks=callbacks_list)
...
while True:
	...
	prompt_padded = np.array([pad_sequences(create_input_sequences(prompt_tokenized), maxlen=max_len, padding="pre", value=vocab['<unk>'])[-1]])
	response = ""
	while True:
