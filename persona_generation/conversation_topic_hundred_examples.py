...
# predicts persona using 100 random samples from Persona-Chat dataset
...
weights_file = 'my_model_weights20_reverse_original.h5'
    ...
    for x in range(maxlen_input - 1):
        ...
        for c in range(len(candidates)):
            if not candidates_finished[c]:
                ye = model.predict([question, candidates[c]])
                yel = ye[0,:]
                # p = np.max(yel)
...
def dbs_decoder(question, b, g, diverse):
    ...
    for t in range(maxlen_input - 1):
        ...
        new_candidates, new_candidate_probabilities, new_candidates_finished = beam_search_step(question, group_candidates[0], 
        ...
        for group in range(1, g):
            new_candidates, new_candidate_probabilities, new_candidates_finished = beam_search_step(question, group_candidates[group], 
...
def beam_search_step(question, candidates, candidate_probabilities, candidates_finished, b_prime, diversity_counts=None, diversity=False):
    new_candidates = []
    new_candidate_probabilities = []
    new_candidates_finished = []
    for c in range(len(candidates)):
...
def tokenize(sentences):
    ...
    if s < (maxlen_input + 1):
        ...
        Q[0,:] = X[- maxlen_input:]
        print(np.asarray(tokenized_sentences)[- maxlen_input:])
...
count = 0
with open("context_movie", "r", encoding="utf-8") as input_file:

    input_array = input_file.readlines()
    # shuffle the deck of inputs
    input_array = np.random.choice(input_array, size=9, replace=False)
    # indices = np.arange(0, len(input_array))
    # np.random.shuffle(indices)
    # input_array = input_array[indices]

    with open("predicted_20_movies", "w", encoding="utf-8") as output_file:
        ...
        for line in input_array:
            ...
            # que = input('user: ')
            query = preprocess(line, name_of_computer)

            line_array = line.split(' ')
            if len(line_array) > 120:
                line = line_array[-120:]
            else:
                line = line_array
            ...
            predout, prob = beam_decoder(Q[0:1], 10)
            ...
            predout_dbs, prob_dbs = dbs_decoder(Q[0:1], 10, 5, 0.3)
            ...
            predout_greedy, prob_greedy = greedy_decoder(Q[0:1])
            ...
            output_file.write("PERSONA BEAM: " + text + "\n")
            output_file.write("PERSONA DBS: " + text_dbs + "\n")
            output_file.write("PERSONA GREEDY: " + text_greedy + "\n\n")
            output_file.flush()
