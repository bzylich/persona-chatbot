...
vocabulary_file = 'vocabulary_personas_clustered'#'vocabulary_personas'
questions_file = 'Padded_context_clustered'#'Padded_context'
answers_file = 'Padded_answers_clustered'#'Padded_answers'
personas_file = 'Padded_personas_clustered'#'Padded_personas'
weights_file = 'my_model_weights20_personas_clustered.h5'#'my_model_weights20_personas.h5'
GLOVE_DIR = '../glove.6B/'
...
def print_result_persona(input, persona_input):

    ans_partial = np.zeros((1,maxlen_input))
    ans_partial[0, -1] = 2  #  the index of the symbol BOS (begin of sentence)
    for k in range(maxlen_input - 1):
        ye = model.predict([input, persona_input, ans_partial])
        mp = np.argmax(ye)
        ans_partial[0, 0:-1] = ans_partial[0, 1:]
        ans_partial[0, -1] = mp
    text = ''
...
merge_layer = Concatenate(axis=1)([topic_embedding, context_embedding, persona_embedding, answer_embedding])
...
p = pickle.load(open(personas_file, 'rb'))
...
eos_id = list(map(lambda v: v[0], vocabulary)).index("EOS")
for m in range(Epochs):
    ...
    for n in range(0,round_exem,step):
        ...
        for i, sent in enumerate(a[n:n+step]):
            l = np.where(sent==eos_id)  #  the position of the symbol EOS
            limit = l[0][0]
            count += limit + 1
        
        print("batch size", count)
        Q = np.zeros((count,maxlen_input))
        P = np.zeros((count,maxlen_input))
        ...
        model.fit([Q, P, A], Y, batch_size=BatchSize, epochs=1, callbacks=callbacks_list)
        # model.fit([Q, A], Y, batch_size=BatchSize, epochs=1, callbacks=callbacks_list)
         
        test_input = qt[41:42]
        test_persona = pt[41:42]
        print(print_result_persona(test_input, test_persona))
        # print(print_result(test_input))
        train_input = q[41:42]
        train_persona = p[41:42]
        print(print_result_persona(train_input, train_persona)) 
