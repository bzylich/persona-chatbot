...
def tokenize(sentences):
    ...
    for w in tokenized_sentences:
        ...
        elif w in word_mapping and word_mapping[w] in word_to_index:
            # use clusters to expand vocabulary
            new_tokenized_sentences.append(word_mapping[w])
        else:
...
# load clusters
word_mapping = {}
with open("vocab_clusters.csv", "r", encoding="utf-8") as cluster_file:
    for line in cluster_file:
        parts = line.strip().split("\t")
        for word in ast.literal_eval(parts[1]):
            word_mapping[word] = parts[0]
...
# persona_selections = random.sample(persona_pool.items()), 3)
persona_items = list(sorted(persona_pool.items()))
persona_selections = []
persona_selections.append(persona_items[36])
persona_selections.append(persona_items[59])
persona_selections.append(persona_items[468])
...
while que != 'goodbye .':
    ...
    # predout_dbs, prob_dbs = dbs_decoder(Q[0:1], selected_persona, 10, 10, 0.3)
